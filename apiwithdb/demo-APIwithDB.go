...
func getCourseList() ([]Course, error) {
	...
	defer cancel()
	results, err := Db.QueryContext(ctx, "SELECT courseid, coursename, price, image_url FROM courseonline")
	if err != nil {
	...
	for results.Next() {
		var course Course
		err = results.Scan(&course.CourseID, &course.CourseName, &course.Price, &course.ImageURL)
		if err != nil {
			log.Println(err.Error())
			return nil, err
		}
...
func getCourse(courseid int) (*Course, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	row := Db.QueryRowContext(ctx, "SELECT courseid, coursename, price, image_url FROM courseonline WHERE courseid = ?", courseid)
	course := &Course{}
...
func corsMiddleware(handler http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Add("Access-Control-Allow-Method", "GET, POST, PUT, DELETE, OPTIONS")
		w.Header().Set("Access-Control-Allow-Headers", "Authorization, Content-Length, Content-Type, Application/JSON, ngrok-skip-browser-warning, User-Agent")
